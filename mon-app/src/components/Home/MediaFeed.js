...
const MediaFeed = () => {
    ...
    useEffect(() => {
        setLoading(true)
        sessionStorage.removeItem("readMedia")
        const mediaCategoryFeed = () => {
            API({
                url: "/posts/media",
                method: "GET",
    ...
    const lastMediaRef = useCallback(element => {
    ...
    return (
        ...
        loading ? <Loader /> :
            <section className="col-12 feedSection">
                ...
                {feed.map((elt, index) =>
                    <article
                        ref={elt.length !== index + 1 ? lastMediaRef : null}
                        key={elt.id}
                        value="media"
                        ...
                        <Link to={`/home/${elt.id}`} onClick={() => sessionStorage.setItem("readMedia", "true")}>
